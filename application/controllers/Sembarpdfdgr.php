<?php
Class Sembarpdfdgr extends CI_Controller{
...
$pdf->Image('assets/dist/img/logopgn.png',0.8,1.0,4.8,1.5);
...
$pdf->Cell(2,0.8, 'Tanggal Pasang', 1, 0, 'C');
$pdf->Cell(2,0.8, 'Tanggal Habis', 1, 0, 'C');
$pdf->Cell(2,0.8, 'Stok', 1, 0, 'C');

$pdf->SetFont('Arial','',7);


 $query =  $this->db->select('semua_barang1.*');

		//join
		$this->db->where('sisa_umur_bulan >=', 0);
        $this->db->where('sisa_umur_bulan <=', 6);
        $query=$this->db->get('semua_barang1')->result();
 ...
 //        foreach ($data_matriks as $row){
    ...
    $pdf->Cell(3,0.8,$row->id_barang,1,0, 'C');   
    $pdf->Cell(4,0.8,$row->nama_barang,1,0, 'C');
    $pdf->Cell(3,0.8,$row->jenis_barang,1,0, 'C');
    $pdf->Cell(2.5,0.8,$row->merk_barang,1,0, 'C');
    $pdf->Cell(2,0.8,$row->tanggal_pasang,1,0, 'C');
    $pdf->Cell(2,0.8,$row->tanggal_habis,1,0, 'C');
    $pdf->Cell(2,0.8,$row->sisa_umur_bulan,1,0, 'C');
}
